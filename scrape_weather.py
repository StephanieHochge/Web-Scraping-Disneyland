...
def get_detailed_and_week_table(tables):
    ...
    for table in tables:
        ...
        if any('(7–10 days)' in str(x) for x in cols_clean):  # check if any of the list items contains the substring
            # (only the table including the forecast for the next week contains the string "(7–10 days)"y)
            week = table.copy()
...
def df_to_arr_of_dtype(df_to_convert, d_type, today=False):
    ...
    :param d_type: the target data type (numpy dtype)
    :param today: if set to true, the DataFrame represents today's weather forecast (bool)
    :return: a numpy array with the target data type and target shape (numpy array)
    """
...
def df_detailed_to_arr(df_both_days, day):
    ...
    try:
        ...
        # convert the DataFrame to a numpy array with the target data type and shape
...
def save_forecasts_this_week(table, file_name):
    ...
    if df_filtered is not None:
        # convert the data frame to an array with the expected data type and shape
