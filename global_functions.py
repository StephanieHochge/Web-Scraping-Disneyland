...
def retrieve_content(url, retrieve_tables=True):
    ...
    :return: the content retrieved from the website (either list of pandas DataFrames or BeautifulSoup object)
    """
    try:
        if retrieve_tables:
            # retrieve the website's html tables
            content = pd.read_html(url)  # see Navlani et al. (2021)
        else:
...
def save_data_to_hdf5(data_to_save, path_to_dset, save_text, file_name,
    ...
    :param file_name: the name of the HDF5 (string)
