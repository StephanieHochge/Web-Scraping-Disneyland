...
def wait_col_to_numeric(df_wait_times):
    """ convert all cells in the DataFrame containing wait times to numerical
...
def scrape_q_times(csv_file, rides_dict):
    ...
    which they are stored in the HDF5 file (dictionary of strings)
    """
    # scrape data from the Queue Times website (see vanden Broucke & Baesens (2018, p. 199))
    ...
    if soup_times is not None:
        ...
        if soup_panels:
            ...
            # check whether the number of retrieved ride names is the same as the number of retrieved wait times
